...
public class TeamPropPositionPipeline_Gold extends OpenCvPipeline {
    ...
    public TeamPropPositionPipeline_Gold(String alliance,double threshold) {
    ...
    public volatile TeamPropPosition position = TeamPropPosition.BLUE_LEFT;
    ...
    public Mat processFrame(Mat input) {
        ...
        if (teamAlliance.equals("BLUE")) {
            if (maxArea == area1) {
                ...
                position = TeamPropPosition.BLUE_MIDDLE;
                Imgproc.rectangle(input, region2_pointA, region2_pointB, GREEN, -1);
            } else {
                position = TeamPropPosition.BLUE_RIGHT;
            ...
            if (maxArea == area1) {
                position = TeamPropPosition.RED_LEFT;
                Imgproc.rectangle(input, region1_pointA, region1_pointB, GREEN, -1);
            } else if (maxArea == area2) {
                position = TeamPropPosition.RED_MIDDLE;
                Imgproc.rectangle(input, region2_pointA, region2_pointB, GREEN, -1);
            } else {
                position = TeamPropPosition.RED_RIGHT;
                Imgproc.rectangle(input, region3_pointA, region3_pointB, GREEN, -1);
            }
            return input;
        }
