package org.firstinspires.ftc.teamcode.Competition.Z20212022FreightFrenzy.Controls.Autonomous.Tank.BlueShippingHubDuckStorageUnit;
...
//                telemetry.addData("LED: ", Bot.pattern);
                ...
                sleep(1000);
                Bot.LyftExtend();
                sleep(525);  // TIME TO EXTEND LYFT - 550 for var distances
                Bot.setBoxHolder_Release();
                sleep(100);  //this allows the servo to lower while the motor is still engaged.
                Bot.LyftStopMotors();
                // Motor retracts easily, so may slide back from sleep.
...
//            Drive to duck spinner.  Should be at it after this.
    ...
    public void DuckSpinnerToStorageUnit (TankBot Bot) {
            ...
            sleep(sleepTime);
            Bot.driveForward(straightSpd, 3.1);
            sleep(sleepTime);
            Bot.rotateLeft(turnEncoderSpd, .11);
            sleep(sleepTime);
            Bot.driveForward(straightSpd, 1.40);
            sleep(sleepTime);
    }
